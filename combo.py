...
def plot_spectrogram_and_rms(ppsd, df, cmap=cmap2, clim=None, grid=True,
        ...
        ax11.plot(trace.times("matplotlib"), trace.data, 'k')
        #print (trace)
        #print (t)
        ax11.set_xlim(t1, t2)
        """
        ...
        ax12.plot(df['Datetime'], df['rms2'], 'b-', linewidth=0.8)
        ...
        if (OUTPUT=="DISP"):
            ax11.set_ylabel('RMS (nm)')
            ax12.set_ylabel('RMS (nm)')
        if (OUTPUT=="VEL"):
            ax11.set_ylabel('RMS (nm/s)')
...
for day in pbar:
    ...
    pbar.set_description("Readind deconvolved data %s" % fn_in)
...
stALL.filter(type='bandpass',freqmin=FMIN, freqmax=FMAX, corners=4)
tr=stALL[0]
"""
...
plot_spectrogram_and_rms(ppsd, dfRMS3, filename="./figures/%s_combo.png"%nslc, show=False,clim=[-160, -120])
exit()

plt.show()
outputfile="./figures/%s.%s.%s.%s.png" % (network,station,location,channel)
print ("outputfile")
